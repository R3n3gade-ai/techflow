...
from tail_hedge import OptionsPosition, run_ptrh_module, PTRHInputs
...
class TestPTRHAutomation(unittest.TestCase):
    ...
    def test_clean_run_no_action(self, mock_execute_roll, mock_execute_correction):
        ...
        ptrh_inputs = PTRHInputs(nav=50_000_000, regime_label="WATCH", regime_score=0.55)
        # Regime table: WATCH = 1.5% NAV = $750k. Actual is $750k.
        positions = [
    ...
    def test_under_hedged_correction(self, mock_execute_roll, mock_execute_correction):
        ...
        ptrh_inputs = PTRHInputs(nav=50_000_000, regime_label="WATCH", regime_score=0.55)
        # Regime table requires $750k. Actual is $600k. Drift = -20% (< -5%).
        positions = [
    ...
    def test_over_hedged_outside_tolerance(self, mock_execute_roll, mock_execute_correction):
        ...
        ptrh_inputs = PTRHInputs(nav=50_000_000, regime_label="WATCH", regime_score=0.55)
        # Regime table requires $750k. Actual is $900k. Drift = +20% (> 15%).
        positions = [
    ...
    def test_over_hedged_inside_tolerance(self, mock_execute_roll, mock_execute_correction):
        ...
        ptrh_inputs = PTRHInputs(nav=50_000_000, regime_label="WATCH", regime_score=0.55)
        # Regime table requires $750k. Actual is $825k. Drift = +10% (< 15%).
        positions = [
